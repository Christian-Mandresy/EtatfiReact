...
import routes from "saisieRoutes.js";

import axios from "axios";
import {  Spinner } from 'reactstrap';

const SaisieLayout = (props) => {
        ...
        <Routes>
          {routes.map((route, index) => (
            <Route
              key={index}
              path={route.path}
              element={userRole === 'saisie' ? <route.component /> : <Navigate to="/auth/login" replace />}
            />
          ))}
        </Routes>
        <Container fluid>
