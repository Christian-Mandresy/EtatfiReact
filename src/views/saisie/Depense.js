...
const Depense = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post('/api/transactions/ajout-depense', formData,{ headers });
      ...
      console.error('Erreur lors de l\'ajout du dépense :', error);
        ...
        <Col>
          <Card className="shadow">
            <CardHeader className="border-0">
              <h3 className="mb-0">Ajout de dépense</h3>
            </CardHeader>
            <CardBody>
              <Form>
                <FormGroup>
                ...
                <Button color="primary" onClick={handleSubmit} disabled={isLoading}>
                  {isLoading ? <Spinner size="sm" color="light" /> : 'Ajouter Depense'}
                </Button>
              </Form>
            </CardBody>
          </Card>
